...
class GPzGaussianCore(tf.Module):
    """
    Gaussian GPz (hetero optional) with the same evidence-style objective as your GPz-t,
    but without IRLS / Student-t. This isolates 'heteroscedastic vs homoscedastic'
    (set hetero True/False).
    """
    ...
    def log_objective_batch(self, Xb: tf.Tensor, yb: tf.Tensor, n_total: tf.Tensor) -> tf.Tensor:
        ...
        wbar, L, A = self.posterior_from_precision(Phi_b, yb, beta_b)
        r = yb - tf.linalg.matmul(Phi_b, wbar)

        quad = -self._half * scale * tf.reduce_sum(beta_b * tf.square(r))
        prior_quad = self._half * tf.reduce_sum(wbar * tf.linalg.matmul(A, wbar))

        logdetB = self._half * scale * tf.reduce_sum(tf.math.log(beta_b + self._eps))
    ...
    def predict(self, Xtr: tf.Tensor, ytr: tf.Tensor, Xte: tf.Tensor, debug: bool = False):
        ...
        if debug:
            tf.print("[GPz-Gaussian] beta_eff test mean:", tf.reduce_mean(beta_s))

        return mean, model_var + noise_var
